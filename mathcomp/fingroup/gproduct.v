...
Canonical depprod_group := FinGroupType extnprod_mulVg.
...
Proof. by rewrite ffunE dfwithin. Qed.

Lemma dfung1_dflt i (g : gT i) j : i != j -> dfung1 g j = 1.
Proof. by move=> ij; rewrite ffunE dfwithout. Qed.
...
Proof.
have [<-|ij] := eqVneq i j; first by rewrite !dfwithin// groupP.
by rewrite !dfwithout // groupP.
Qed.
...
by apply/val_inj; have [<-|nij]/= := eqVneq i j;
   [rewrite !dfwithin|rewrite !dfwithout].
Qed.
...
apply/imsetP/familyP => [[g gG -> j]|xG]; rewrite ?ffunE.
  by have [<-|ij] := eqVneq i j; [rewrite !dfwithin|rewrite !dfwithout].
exists (x i); first by have := xG i; rewrite dfwithin.
apply/ffunP => j /[!ffunE]; have := xG j.
have [<-|nij] := eqVneq i j; first by rewrite !dfwithin.
by rewrite !dfwithout//; apply/set1P.
Qed.
...
apply/setXnP => j /[!ffunE]; have [<-|ij] := eqVneq i j.
  by rewrite !dfwithin xH.
by rewrite !dfwithout.
Qed.
...
Proof.
rewrite -setXn_prod//=.
pose G i := setXn_group (group_dfwith (fun=> [1 gT _]%G) (H i)).
have /= /introT/(_ _)/eqP->//= := @bigdprodYP _ _ xpredT G.
  rewrite bigprodGE.
  admit.
move=> i _; rewrite bigprodGE subsetD/= gen_subG.
Admitted.
