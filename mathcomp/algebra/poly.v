...
Lemma lead_coef_XnaddC n c : 0 < n -> lead_coef ('X^n + c%:P) = 1.
Proof.
move=> n_gt0; rewrite lead_coefDl ?lead_coefXn//.
by rewrite size_polyC size_polyXn ltnS (leq_trans (leq_b1 _)).
Qed.

Lemma lead_coef_XaddC c : lead_coef ('X + c%:P) = 1.
Proof. exact: (@lead_coef_XnaddC 1%N). Qed.

Lemma lead_coef_XnsubC n c : 0 < n -> lead_coef ('X^n - c%:P) = 1.
Proof. by move=> n_gt0; rewrite -polyCN lead_coef_XnaddC. Qed.

Lemma lead_coef_XsubC c : lead_coef ('X - c%:P) = 1.
Proof. exact: (@lead_coef_XnsubC 1%N). Qed.
...
Proof. by move=> n_gt0; rewrite monicE lead_coef_XnaddC. Qed.

Lemma monicXnsubC n c : 0 < n -> 'X^n - c%:P \is monic.
Proof. by move=> n_gt0; rewrite monicE lead_coef_XnsubC. Qed.
...
#[deprecated(since="mathcomp 1.16",note="Use size_XnsubC instead.")]
Lemma size_Xn_sub_1 n : n > 0 -> size ('X^n - 1 : {poly R}) = n.+1.
Proof. exact/size_XnsubC. Qed.

#[deprecated(since="mathcomp 1.16",note="Use monicXnsubC instead.")]
...
Lemma polyOverXn n : 'X^n \in polyOver kS.
...
Lemma polyPverXnsubC n c : ('X^n - c%:P \is a polyOver kS) = (c \in kS).
...
Lemma prim_root_natf_eq0 : n%:R != 0 :> R.
...
Lemma eq_in_map_poly_id0 (f g : aR -> rR)
...
Lemma eq_in_map_poly (f g : {additive aR -> rR})
  (S0 : {pred aR}) (addS : addrPred S0) (kS : keyed_pred addS) :
  {in kS, f =1 g} -> {in polyOver kS, map_poly f =1 map_poly g}.
